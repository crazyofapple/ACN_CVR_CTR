...
class Transformer(Layer):
    ...
    def call(self, inputs, mask=None, training=None, **kwargs):
        ...
        return result  # tf.reduce_mean(result, axis=1, keep_dims=True)
...
class DynamicGRU(Layer):
    def __init__(self, num_units=None, gru_type='GRU', return_sequence=True, supports_masking=False, **kwargs):
...
# # Capsule(10, 16, 3, True)(cnn)
# class Capsule(Layer):
#     def __init__(self, num_capsule, dim_capsule, routings=3, kernel_size=(9, 1), share_weights=True,
#                  activation='default', supports_masking=False, **kwargs):
...
#                                      dtype=tf.float32,
#                                      initializer=tf.keras.initializers.glorot_uniform(2019),
#                                      trainable=True)
#
#         else:
...
#         if self.share_weights:
#             u_hat_vecs = K.conv1d(u_vecs, self.W, data_format="channels_last")
#         else:
...
class Capsule(Layer):
    ...
    def build(self, input_shape):
        super(Capsule, self).build(input_shape)
        input_dim_capsule = input_shape[-1].value
        # if self.share_weights:
        #     self.W = self.add_weight(name='capsule_kernel',
        #                              shape=(1, input_dim_capsule,
        #                                     self.num_capsule * self.dim_capsule),
        ...
        #
        # else:
        #     input_num_capsule = input_shape[-2].value
        #     self.W = self.add_weight(name='capsule_kernel',
        #                              shape=(input_num_capsule,
        #                                     input_dim_capsule,
        #                                     self.num_capsule * self.dim_capsule),
